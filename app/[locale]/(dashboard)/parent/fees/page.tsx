...
export default async function Page({
  ...
  if (children.length === 0) {
    return (
      <>
        ...
        <EmptyState message={t("noSchedule")} />
      </>
    );
  }
  ...
  const childId =
    (typeof sp.child === "string" && children.some((c) => c.id === sp.child) ? sp.child : null) ?? children[0].id;
    ...
    <>
      ...
      {children.length > 1 ? (
        <div className="mb-4 flex flex-wrap gap-2">
          {children.map((c) => (
            <a key={c.id} href={`?child=${c.id}`}
              className={cn("rounded-full border px-3 py-1 text-sm", c.id === childId ? "border-[var(--brand)] bg-[var(--brand)] text-white" : "border-[var(--border)] hover:bg-black/[0.03]")}>
              {name(c.user)}
            </a>
          ))}
        </div>
      ) : null}
      <StudentFeeView studentId={childId} locale={locale} manage={false} />
    </>
  );
}
